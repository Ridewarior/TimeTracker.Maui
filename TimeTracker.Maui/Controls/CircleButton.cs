using TimeTracker.Maui.Controls.Drawables;

namespace TimeTracker.Maui.Controls;

public class CircleButton : GraphicsView
{
    /// <summary>
    ...
    public static BindableProperty ButtonColorProperty = BindableProperty.Create(
        nameof(ButtonColor), typeof(Color), typeof(CircleButton), propertyChanged: OnButtonColorChanged);

    public static BindableProperty ImageProperty = BindableProperty.Create(
        nameof(Image), typeof(string), typeof(CircleButton), propertyChanged: OnImageChanged);

    public new static BindableProperty IsVisibleProperty = BindableProperty.Create(
        nameof(IsVisible), typeof(bool), typeof(CircleButton), defaultValue: false, propertyChanged: OnIsVisibleChanged);

    public CircleButton()
    {
        //Handler = new CircularButtonHandler();
        var drawable = new CircleButtonDrawable();
        Drawable = drawable;
    }
    ...
    static void OnButtonColorChanged(BindableObject bindable, object oldValue, object newValue)
    {
        ...
        if (control.Drawable is CircleButtonDrawable thisDrawable)
